      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            {entry.type === 'work' ? (
              <Badge variant="default" className="text-[10px] px-1.5 py-0">Work</Badge>
            ) : (
              <Badge variant="secondary" className="text-[10px] px-1.5 py-0">Leave</Badge>
            )}
          </div>
            ...
            {entry.type === 'work' ? (
              <>
                ...
                {entry.sessions.length > 1 && (
                  <span>({entry.sessions.length} sessions)</span>
                )}
              </>
            ) : (
          ...
          >
            <div className="border-t border-border/30 px-4 py-3 space-y-3">
                ...
                {editingNotes ? (
                  <div className="flex-1 flex items-center gap-1">
                    <Input
                      ...
                      autoFocus
                    />
                    <Button size="icon" variant="ghost" className="h-7 w-7 shrink-0" onClick={saveNotes}>
              ...
              {entry.sessions.map((session, idx) => (
                <div
                  key={session.id}
                  className="rounded-lg bg-muted/30 border border-border/20 p-3 space-y-2"
                >
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-1">
                      {editingSessionId === session.id ? (
                        <>
                          <Button size="icon" variant="ghost" className="h-7 w-7" onClick={saveSessionEdit}>
                            <Check className="h-3 w-3 text-primary" />
                          </Button>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <label className="text-[10px] uppercase tracking-wider text-muted-foreground flex items-center gap-1">
                        <Coffee className="h-3 w-3" /> Break Start
                      </label>
                      {editingSessionId === session.id ? (
                        <Input
                          type="time"
                    ...
                    <div>
                      <label className="text-[10px] uppercase tracking-wider text-muted-foreground flex items-center gap-1">
                        <Coffee className="h-3 w-3" /> Break End
                      </label>
                      {editingSessionId === session.id ? (
                        <Input
                          type="time"
