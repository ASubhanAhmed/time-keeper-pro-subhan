...
export const EntryCard = memo(function EntryCard({ entry, onUpdate, onDelete, onUpdateSession, onDeleteSession }: EntryCardProps) {
  ...
  const startEditSession = useCallback((session: WorkSession) => {
  ...
  }, [editingSessionId, editValues, entry.id, onUpdateSession]);

  const cancelEdit = useCallback(() => { setEditingSessionId(null); setEditValues({}); }, []);

  const saveNotes = useCallback(() => { onUpdate(entry.id, { notes: notesValue }); setEditingNotes(false); }, [entry.id, notesValue, onUpdate]);

  const toggleExpand = useCallback(() => setIsExpanded(prev => !prev), []);

  return (
    <div className="rounded-2xl border border-border/30 bg-card shadow-sm overflow-hidden">
      <div className="flex items-center gap-3 px-4 py-3 cursor-pointer hover:bg-muted/20 transition-colors" onClick={toggleExpand}>
