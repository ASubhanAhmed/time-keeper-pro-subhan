...
export function StatusCard({ status, todayEntry }: StatusCardProps) {
  const getStatusBadge = () => {
    if (!status.isClockedIn) {
      return <Badge variant="secondary" className="text-sm rounded-lg">Not Working</Badge>;
    }
    if (status.isOnBreak) {
      return <Badge className="bg-accent text-accent-foreground text-sm rounded-lg">On Break</Badge>;
    }
    return <Badge className="bg-primary text-primary-foreground text-sm rounded-lg">Working</Badge>;
  };
  ...
  return (
    <Card className="border-none glass rounded-2xl shadow-lg">
      ...
      <CardContent className="space-y-4">
        <div className="grid grid-cols-2 gap-3 sm:gap-4">
          <div className="flex items-center gap-2 sm:gap-3 rounded-xl bg-background/60 p-2 sm:p-3">
